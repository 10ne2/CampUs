...
<div style="height: 900px; padding: 15px;">
	...
	<div class="row" style="flex-direction: row;">
		<div class="col-md-2" style="width: 250px">
			<a class="btnw btn-primary btn-block mb-3" style="width: 245px;"
				onclick="mailWrite()">메일 작성</a>
			<div class="cardc" style="width: 250px; height: 80px">
				<div class="" style="width: 100%">
					<div
						style="width: 245px; display: flex; text-align: center; flex-direction: row; justify-content: center; gap: 0px;">
						<button id="btnUnread" type="button" class="mailT"
							style="width: 100px; height: 70px; list-style: none; border: none;">
							<span
								style="display: block; text-align: center; font-size: 25px; color: #22A99C; font-weight: bold;">${unreadCount }</span>
						...
						<button id="btnStar" type="button" class="mailT"
							...
							<img id="imgStar"
								src="<%=request.getContextPath()%>/resources/images/imp.png"
								style="width: 30px; margin: 0 0 5px -1px"></img> <span
								style="display: block; font-size: 15px">중요</span>
						</button>
						<button id="btnAtt" type="button" class="mailT"
							style="width: 100px; height: 70px; text-align: center; list-style: none; border: none; margin-top: 4px">
							<img id="imgAtt"
								src="<%=request.getContextPath()%>/resources/images/att.png"
								style="width: 30px; margin: 0 0 5px -1px"></img> <span
			...
			<div class="card" style="width: 245px;">
				...
				<div class="card-body p-0" style="width: 245px;">
					<ul class="nav flex-column" style="height: 658px;">
						<li class="" style="width: 100%; height: 50px;">
							<button id="btnAll" type="button"
								class="d-flex align-items-center mailR"
								style="width: 100%; height: 100%; gap: 20px; line-height: 50px; border: none; padding: 15px; overflow: hidden">
								<i class="fas fa-inbox" style=""></i> <span
									style="display: block;">전체 메일함</span>
							</button>
						</li>
						<li class="nav-item" style="width: 100%; height: 50px">
							<button id="btnRecv" type="button"
								class="d-flex align-items-center mailR"
								style="width: 100%; height: 100%; gap: 20px; line-height: 50px; border: none; padding: 15px">
								<i class="far fa-envelope" style=""></i> <span
									style="display: block;">받은 메일함</span> <span
									class="badgec bg-primaryc"
									style="width: auto; display: block; margin-left: auto; padding: 0 5px 0 5px">${unreadCount}</span>
							</button>
						</li>
						<li class="nav-item"
							style="height: 50px; border-bottom: 1px solid #ddd;">
							<button id="btnSent" type="button"
								class="d-flex align-items-center mailR"
								style="width: 100%; height: 100%; gap: 24px; line-height: 50px; border: none; padding: 15px">
								<i class="far fa-file-alt" style="margin-left: 2px"></i> <span
									style="display: block; margin-left: -2px">보낸 메일함</span>
							</button>
						</li>
					</ul>
				</div>
					...
					<div class="card-tools">
						<div class="input-group input-group-sm">
							<input type="text" class="form-control"
								placeholder="검색어를 입력해주세요." value="${pageMaker.keyword }">
								...
								<c:if test="${not empty mailList }">
									<c:forEach items="${mailList }" var="mail">
										<tr
											...
											data-sender="${mail.mail_sender}"
											data-receiver="${mail.mail_receiver}"
											data-unread="${mail.mail_receiver == loginUser.mem_id and (mail.mail_read == '0')}"
											data-star="${mail.mail_important}" data-att="">
											<td
												style="width: 100%; min-height: 60px; display: flex; flex-direction: column;">
												<div
													style="width: 100%; display: flex; flex-direction: row;">
													<div class="icheck-primary" style="width: 48px">
														<input type="checkbox" value="${mail.mail_id}"
															id="check_${mail.mail_id}"> <label
															for="check_${mail.mail_id}"></label>
													</div>
													<div class="" style="">
														<img id="readImg"
															src="<%=request.getContextPath()%>/resources/images/${mail.mail_read == 0 ? 'nread' : 'read'}.png"
															style="width: 20px; cursor: pointer" />
													</div>
													<div class=""
														style="width: 150px; display: flex; flex-direction: row; margin-left: 10px">
														<a style="line-height: 30px;"> ${mail.mail_sender == sessionScope.loginUser.mem_id ? mail.receiver_name : mail.sender_name}
														</a> <a
															style="line-height: 30px; margin-left: 15px; font-size: 14px; color: #999">
															${mail.mail_sender == sessionScope.loginUser.mem_id ? mail.mail_receiver : mail.mail_sender}
														</a>
													</div>
													<div class="mailbox-date"
														style="margin-left: auto; line-height: 30px; font-size: 12px; color: #bbb">
														<fmt:formatDate
															value="${mail.mail_sender == sessionScope.loginUser.mem_id ? mail.mail_rdate : mail.mail_sdate}"
															pattern="yy-MM-dd HH:mm" />
													</div>
												</div>
											</td>
											<td
												style="min-height: 55px; border: none; display: flex; flex-direction: row; margin-top: -25px">
												<div class="mailbox-subjectc"
													style="margin-left: 60px; overflow: hidden; text-overflow: ellipsis;">
													<a style="font-size: 14px; color: #999">
														${mail.mail_sender == sessionScope.loginUser.mem_id ? "[받은메일함]" : "[보낸메일함]"}
													</a> <a style="font-size: 14px;">${mail.mail_name }</a>
												</div>
												<div class="mailbox-star" style="margin-left: auto;">
													<img id="starImg"
														src="<%=request.getContextPath()%>/resources/images/${mail.mail_important == 0 ? 'imp' : 'imp_act'}.png"
														style="width: 20px; cursor: pointer" onclick="starClick()" />
												</div>
											</td>
										</tr>
									</c:forEach>
		...
		<!-- ------------상세-------------- -->
		<div class="col-md-7 mailDetail" style="width: 768px; display: block">
			<div class="card card-primaryc card-outline"
				...
				<div class="card-body p-0">
					<div class="mailbox-read-info" style="padding: 15px">
						<div style="display: flex; flex-direction: row;">
							<h5 style="margin-bottom: 15px">전자정부 풀스택 503호 1조(시스아웃) ERD 및
								화면정의서 제출합니다.</h5>
							<span class="mailbox-read-time float-right"
								style="display: block; width: 200px; margin-top: 12px; margin-left: auto; text-align: right;">2025년
								7월 19일 오후 3:07</span>
						</div>
						<div style="display: flex; flex-direction: row;">
							<span style="width: 80px; display: block; line-height: 28px">받는
								사람</span>
							<div
								style="height: 30px; background-color: #DFFCF9; border-radius: 15px; display: flex; flex-direction: row;">
								<span
									style="display: block; margin-left: 15px; line-height: 28px">김원희</span>
								<span
									style="display: block; margin-left: 15px; line-height: 28px; margin-right: 15px">&lt;rla9513@naver.com&gt;</span>
							</div>
				...
				<div class="card-footer bg-white"
					...
					<a href="#" download class="btn btn-default"> <i
						class="fas fa-paperclip"></i> Attachment
					</a> <span style="display: block; margin-left: 10px; line-height: 38px">1,234
						KB</span>
				</div>
			</div>
			<!-- /.card-footer -->
			...
			<div class="card card-primaryc card-outline" style="height: 810px;">
				<div class="card-header" style="height: 50px">
					...
					<div class="float-right">
						<button type="reset" class="btn btn-default"
							style="height: 35px; margin-top: -5px; line-height: 5px"
							onclick="closeWrite()">
							<i class="fas fa-times"></i>창닫기
						</button>
				...
				<div class="card-body">
					<div class="form-group" style="display: flex; flex-direction: row;">
						<span style="display: block; width: 8%; line-height: 32px">받는
							사람</span> <input class="form-control" placeholder="받는 사람을 입력해주세요.">
					</div>
					<div class="form-group" style="display: flex; flex-direction: row;">
					...
					<div class="form-group" style="display: flex-direction: row;">
						<div class="btn btn-default btn-file">
							<i class="fas fa-paperclip"></i> 파일 선택 <input type="file"
								name="attachment">
						</div>
						<p class="help-block"
							style="margin-left: 15px; margin-top: 13px; line-height: 10px">Max.
							32MB</p>
					</div>
				</div>
				<!-- /.card-body -->
